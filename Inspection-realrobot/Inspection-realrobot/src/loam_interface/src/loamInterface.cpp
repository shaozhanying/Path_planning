...
string fusionScanTopic = "/fusion_cloud";
...
void odometryHandler(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  if (flipStateEstimation) {
    ...
    // pitch = roll;
    // yaw = -yaw;
    // geoQuat = tf::createQuaternionMsgFromRollPitchYaw(pitch,-yaw, roll-PI/2);
    geoQuat = tf::createQuaternionMsgFromRollPitchYaw(roll,pitch,yaw);

    odomData.pose.pose.orientation = geoQuat;
    double temp =odomData.pose.pose.position.x;
    odomData.pose.pose.position.x = odom->pose.pose.position.z;
    odomData.pose.pose.position.y = -temp;
  ...
  // publish tf messages
  odomTrans.stamp_ = ros::Time::now();//odom->header.stamp;
...
void scanCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn)
{
  pubLaserCloudPointer->publish(laserCloudIn);
}
...
int main(int argc, char** argv)
{
  ...
  // nhPrivate.getParam("fusionScanTopic", fusionScanTopic);
  ...
  // ros::Subscriber subScanCloud = nh.subscribe<sensor_msgs::PointCloud2> (fusionScanTopic, 5, scanCloudHandler);
