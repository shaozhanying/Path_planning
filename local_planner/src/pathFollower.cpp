...
void pathHandler(const nav_msgs::Path::ConstPtr& pathIn)
{
  ...
  //   ROS_INFO("%f %f %f",path.poses[i].pose.position.x,path.poses[i].pose.position.y,path.poses[i].pose.position.z);
...
int main(int argc, char** argv)
{
  ...
  while (status) {
    ...
    if (pathInit) {
      //小车已经移动了很小的一段距离，这个算出了偏移的距离
      float vehicleXRel = cos(vehicleYawRec) * (vehicleX/*当前坐标*/ - vehicleXRec/*前一时刻坐标*/) 
      ...
      //找到大于lookaheaddis的路径点
      while (pathPointID < pathSize - 1) {
        disX = path.poses[pathPointID].pose.position.x - vehicleXRel;
        disY = path.poses[pathPointID].pose.position.y - vehicleYRel;
        ROS_INFO("%lf",vehicleXRel);
        dis = sqrt(disX * disX + disY * disY);
        if (dis < lookAheadDis/*0.5*/) {
          pathPointID++;
          ROS_INFO("%d",pathPointID);
        } else {
          break;
        }
      }
      ...
      float joySpeed2 = maxSpeed /*2.0*/* joySpeed;
      if (!navFwd) {
        dirDiff += PI;
        if (dirDiff > PI) dirDiff -= 2 * PI;
        joySpeed2 *= -1;
