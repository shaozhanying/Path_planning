...
float gridVoxelOffsetX = 3.6;
float gridVoxelOffsetY = 6.5;
const int gridVoxelNumX = 181;
const int gridVoxelNumY = 651;
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubPath = nh.advertise<nav_msgs::Path> ("/pat", 0);
  nav_msgs::Path path1;
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (selectedGroupID >= 0) {
          ...
          path1.poses.resize(selectedPathLength);
          for (int i = 0; i < selectedPathLength; i++) {
            ...
            if (dis <= pathRange / pathScale && dis <= relativeGoalDis / pathScale) {
              path1.poses[i].pose.position.x = pathScale * (cos(rotAng) * x - sin(rotAng) * y);
              path1.poses[i].pose.position.y = pathScale * (sin(rotAng) * x + cos(rotAng) * y);
              path1.poses[i].pose.position.z = pathScale * z;
            } else {
              path1.poses.resize(i);
              break;
            }
          }

          path1.header.stamp = ros::Time().fromSec(odomTime);
          path1.header.frame_id = "vehicle";
      ...
      if (!pathFound) {
        path1.poses.resize(1);
        path1.poses[0].pose.position.x = 0;
        path1.poses[0].pose.position.y = 0;
        path1.poses[0].pose.position.z = 0;
    ...
    for(int i=0;i<plannerCloudCropSize;i++)
    {
      ...
      if((fabs(x)<vehicleLength / 2.0 && fabs(y) <vehicleWidth / 2.0 ) && h> obstacleHeightThre)
        {
          ROS_INFO("accident!!!\n");
          break;
        }
