...
int main(int argc, char **argv)
{
  ...
  while (status)
  {
    ...
    if (pathInit)
    {
      ...
      {
        if ((!stoprotation && abs(endDis) > 0.05) || (stoprotation && abs(endDis) > 0.1))
        {
          vehicleSpeed = endDis;
          if (vehicleSpeed > 1.0)
            vehicleSpeed = 1.0;
          else if (vehicleSpeed < -1.0)
            vehicleSpeed = -1.0;
        ...
        // std::cout<<"dis "<<dis<<" endDis "<<endDis<<"dirDiff "<<dirDiff/M_PI*180<<" pathDir "<<pathDir/M_PI*180<<" dirDiffend "<<dirDiffend<<" endDisYaw "<<endDisYaw<<std::endl;
      }
      else
      {
        stoprotation = false;
        ...
        // std::cout<<"dirDiff "<<dirDiff<<std::endl;
        // std::cout<<"joySpeed2 "<<joySpeed2<<" enddis "<<endDis<<std::endl;
      ...
      if (safetyStop >= 2)
        vehicleYawRate = 0;
      // std::cout<<"vehicleYawRate "<<vehicleYawRate<<" vehicleSpeed "<<vehicleSpeed<<std::endl;
      ...
      if (pubSkipCount < 0)
      {
        ...
        if (closeObstacle || disObstacle)
        {
          if( cmd_vel.twist.linear.x / 2>0.1)
            cmd_vel.twist.linear.x = cmd_vel.twist.linear.x / 2;
        }
